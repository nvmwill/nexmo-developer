...
The OpenTok platform leverages the WebRTC protocol and the real-time video codecs that are supported by WebRTC. In particular the OpenTok platform supports the VP8 and the H.264 video codecs. You can select which video codec you want to assign as your preferred codec for a particular OpenTok project. Across the broad ecosystem of devices and browsers that OpenTok supports there are varying levels of support for the VP8 and H.264 real-time video codecs. Some endpoints support both video codecs, and some just support one video codec. Depending on the type of application you are building and the types of browsers and devices your end users will use, your choice of preferred codec will change.
...
The VP8 real-time video codec is a software codec. It can work well at lower bitrates and is a mature video codec in the context of WebRTC. As a software codec it can be instantiated as many times as is needed by the application within the limits of memory and CPU. The VP8 codec supports the [OpenTok Scalable Video](/developer/guides/scalable-video) feature, which means it works well in large sessions with supported browsers and devices. The H.264 real-time video codec is available in both hardware and software forms depending on the device. It is a relatively new codec in the context of WebRTC although it has a long history for streaming movies and video clips over the internet. Hardware codec support means that the core CPU of the device doesn’t have to work as hard to process the video, resulting in reduced CPU load. The number of hardware instances is device-dependent with iOS having the best support. Given that H.264 is a new codec for WebRTC and each device may have a different implementation, the quality can vary. As such, H.264 may not perform as well at lower bit-rates when compared to VP8. H.264 is not well suited to large sessions since it does not support the OpenTok Scalable Video feature.

## Setting the preferred video codec for a project

You can set the preferred video codec for a project on the Project page of your [Video API account](https://tokbox.com/account/). In [routed sessions](/developer/guides/create-session/#media-mode) (sessions that use the OpenTok Media Router), the preferred video codec is used for all clients in the session. In [relayed sessions](/developer/guides/create-session/#media-mode), clients send streams directly to one another, and each publishing-subscribing pair tries to find a common video codec that they can both use. In this regard, the video codec used by the pair may be different to the preferred video codec setting.
...
![](/developer/img/docs/video-codec-overview.png)  
...
The [OT.getSupportedCodecs()](/developer/sdks/js/reference/OT.html#getSupportedCodecs) method returns a Promise that is resolved (on success) with an object that has two properties: `videoDecoders`, an array of supported video codecs for decoding, and `videoEncoders`, an array of supported video codecs for encoding. The following example gets the list of supported codecs for encoding and decoding video streams:
...
The [MediaUtils.SupportedCodecs.getSupportedCodecs(context)](/developer/sdks/android/reference/com/opentok/android/MediaUtils.html#getSupportedCodecs-android.content.Context-) returns a [MediaUtils.SupportedCodecs](/developer/sdks/android/reference/com/opentok/android/MediaUtils.SupportedCodecs.html) object that that has two properties: `videoDecoders`, an ArrayList of supported video codecs (defined by the [MediaUtils.VideoCodecType](/developer/sdks/android/reference/com/opentok/android/MediaUtils.VideoCodecType.html) class) for decoding, and `videoEncoders`, an ArrayList of supported video codecs (defined by the MediaUtils.VideoCodecType class) for encoding. The following example gets the list of supported codecs for encoding and decoding video streams:
